...
class CommandManager {
	...
	processCommand(bot, msg, config, settingsManager) {
		var doAsync = async () => {
			...
			if (command !== null) {
				if (msg.channel.guild !== undefined && !msg.channel.permissionsOf(msg.author.id).has('manageChannels') && await(settingsManager.isCommandIgnored(this.prefix, command.name, msg.channel.guild.id, msg.channel.id, msg.author.id)) === true)
					return;
